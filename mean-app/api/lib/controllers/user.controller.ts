...
class UserController implements Controller {
    ...
    private authenticate = async (request: Request, response:
        ...
        try {
            ...
            if (!user) {
                return response.status(401).json({ error: 'Unauthorized (UserNotFound)' });
            }

            await this.passwordService.authorize(user.id, await this.passwordService.hashPassword(password));
            
            try {
                await this.tokenService.remove(user.id); // delete previous tokens
            }  catch (error) {
                console.error(`Login (delete previous tokens): ${error.message}`);
            }
            ...
            response.status(401).json({ error: 'Unauthorized (Error)' });
        }
    };
